...
const FlightAdmin = () => {
  ...
  const fetchData = async () =>{
    ...
    await axios.get('http://localhost:6001/fetch-flights').then(
      (response)=>{
        const operatorFlights = response.data.filter(flight => flight.operatorId === userId);
        setFlightsCount(operatorFlights.length);
        
        // After getting flights, fetch bookings
        axios.get('http://localhost:6001/fetch-bookings').then(
          (bookingsResponse)=>{
            const operatorFlightIds = operatorFlights.map(f => f._id);
            const operatorBookings = bookingsResponse.data.filter(booking => operatorFlightIds.includes(booking.flight));
            setbookingCount(operatorBookings.length);
          }
        );
      }
    );
